...
int handle_dollar(char **new_token, char *string, int index, char **env)
{
	...
	i = index;
	len = 0;
	while (string[i] && !is_space(string[i]) && !is_quote(string[i]) && (ft_isalnum(string[i]) || string[i] == '?'))
	{
		i++;
		len++;
	}
...
int handle_double_quotes(char **new_token , char *string, int index, char **env)
{
	int i;
	char *skip;
	int len;
	...
	if (string[i] == '$')
		i = handle_dollar(new_token, string, i, env);
	return (i);
}

void join_double_quotes(char **new_token, char *string)
{
	int i = 0;
	int len = 0;
	char *skip;
	...
	skip = ft_substr(string, i - len - 1, len);
	*new_token = ft_strjoin(*new_token, skip);
	free(skip);
}
...
void expander(char **tokens, char **env)
{
	...
	while (tokens[j])
	{
		...
		while (*string)
		{
			...
			{
				new_token = append_character(&new_token, string[i]);
				i++;
			}
			if(string[i] == '$')
